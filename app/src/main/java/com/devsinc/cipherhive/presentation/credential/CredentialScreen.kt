...
fun CredentialScreen(
    ...
    LaunchedEffect(key1 = context) {
        ...
        if (updating && credential != null) {
            viewModel.updateState(credential)
        }
        ...
        viewModel.validationEvents.collect { event ->
            when (event) {
                is CredentialViewModel.ValidationEvent.Success -> {
                    ...
                    Toast.makeText(
                        context,
                        "Credential ${if (updating) "updated" else "saved"} successfully",
                        Toast.LENGTH_SHORT
                    ).show()
                }
    ...
    Column(
        ...
        Button(
            onClick = {
                if (updating) viewModel.onEvent(
                    CredentialFormEvent.OnUpdateClicked(
                        credential!!
                    )
                ) else viewModel.onEvent(CredentialFormEvent.OnSaveClicked)

            }, modifier = Modifier.fillMaxWidth(), colors = ButtonDefaults.buttonColors(
                containerColor = Color(0xFFFF6464)
            )
        ) {
            Text(text = if (updating) "Update" else "Save")
        }
    }
}
